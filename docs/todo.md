# Requirements to do implement
This section list the missing features and issues for this project, in order of priority.

## Robot Features
- PID implementation on robot
- UDP Communication Backend and simulation implementation (Real implementation are done by BlueTooth)
- Change enviroment via file reading (include name on manifest) [Mostly Done]
- Scene atributes via file reading (include name on manifest)
- Improve simulation architecture (physics, match rules, etc...)
- UI Observability
- Suscribe robot to game to avoid passing as argument

## Current issues
- _Modularize run system, example (run simulation, run robot, show cameras, process test (ui, logger, etc...)) [Mostly Done]_
